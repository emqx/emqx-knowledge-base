...
<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="p-8 bg-white rounded-lg shadow-md max-w-md w-full">
      <h1 class="mb-6 text-2xl font-bold text-center text-indigo-600">EMQX Knowledge Base</h1>
      ...
      <div v-else-if="!isAuthenticated">
        <p class="mb-6 text-center text-gray-600">Please sign in with GitHub to access the knowledge base.</p>
        ...
        <div v-if="error" class="mt-4 p-3 text-sm text-red-600 bg-red-50 rounded-md">
          {{ error }}
        </div>
        
        <div class="mt-6 text-xs text-gray-500">
          <p>You need to be a member of the specified GitHub organization to access this application.</p>
        </div>
      </div>
    </div>
  </div>
